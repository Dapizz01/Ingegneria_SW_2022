...
public class LavoratoriDaoImpl implements LavoratoriDao {
    ...
    public boolean addLavoratore(Lavoratore nuovoLavoratore) {
        ...
        try {
            ...
            System.out.println("Opened database successfully<LavoratoriDaoImpl>");
            //---------------------------------------------------
            ...
            for (Esperienza esperienza : nuovoLavoratore.getEsperienze()) { //Cicla in base alle esperienze che ha quel nuovo lavoratore da inserire
                sql = "INSERT INTO Esperienze (NomeAzienda,RetribuzioneGiornaliera,InizioPeriodo,FinePeriodo,NomeComune,NomeSpecializzazione) VALUES (?,?,?,?,?,?)";
            ...
            * viene inserito nel DB prenderà l'ID con il numero più alto visto che è autoincrementale*/
            ...
            for (Contatto nuovoContatto : nuovoLavoratore.getContatti()) {
                sql = "INSERT INTO Contatti (NomeContatto,CognomeContatto,N_telefono,Email) VALUES (?,?,?,?)";
                pstmt = c.prepareStatement(sql);
             ...
             * viene inserito nel DB prenderà l'ID con il numero più alto visto che è autoincrementale*/
            stmt = c.createStatement();
            rs = stmt.executeQuery("SELECT ID_Lavoratore FROM Lavoratori ORDER BY ID_Lavoratore DESC LIMIT 1;"); // Cerca l'ultimo ID inserito nella tabella lavoratori(ovvero quello che sto inserendo)

            idLavoratore = rs.getInt("ID_Lavoratore");
